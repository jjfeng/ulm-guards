...
class ParallelWorkerManager:
    ...
    def create_batch_worker_cmds(self, worker_list, num_approx_batches):
        ...
        shared_file_name = "%s/shared.pkl" % self.worker_folder
        with open(shared_file_name, "wb") as shared_file:
            pickle.dump(
                self.shared_obj,
                shared_file,
                protocol=-1
        ...
        for batch_idx, start_idx in enumerate(range(0, num_workers, num_per_batch)):
            ...
            with open(input_file_name, "wb") as cmd_input_file:
                ...
                pickle.dump(
                    BatchParallelWorkers(batched_workers),
                    cmd_input_file,
                    protocol=-1
                )
            cmd_str = "python3 run_worker.py --input-file %s --shared-file %s --output-file %s --log-file %s" % (input_file_name, shared_file_name, output_file_name, log_file_name)
            print(cmd_str)
...
class BatchSubmissionManager(ParallelWorkerManager):
    ...
    def create_batch_worker_cmds(self, worker_list, shared_obj, num_approx_batches, worker_folder):
        ...
        """
        self.shared_obj = shared_obj
        shared_file_name = "%s/shared.pkl" % worker_folder
        with open(shared_file_name, "wb") as shared_file:
            # Pickle the worker as input to the job
            pickle.dump(
                shared_obj,
        ...
        for batch_idx, start_idx in enumerate(range(0, num_workers, num_per_batch)):
            ...
            cmd_str = "python %s/run_worker.py --input-file %s --shared-file %s --output-file %s" % (os.path.dirname(os.path.realpath(__file__)), input_file_name, shared_file_name, output_file_name)
            batch_cmd = CustomCommand(
                cmd_str,
                outfname=output_file_name,
                logdir=worker_batch_folder,
